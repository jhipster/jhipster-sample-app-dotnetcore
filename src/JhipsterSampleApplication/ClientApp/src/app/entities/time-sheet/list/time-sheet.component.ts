...
export class TimeSheetComponent implements OnInit {
  ...
  timeSheets = signal<ITimeSheet[]>([]);
  isLoading = false;

  sortState = sortStateSignal({});

  public readonly router = inject(Router);
  protected readonly timeSheetService = inject(TimeSheetService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: ITimeSheet): string => this.timeSheetService.getTimeSheetIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.timeSheets().length === 0) {
            ...
            this.timeSheets.set(this.refineData(this.timeSheets()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.timeSheets.set(this.refineData(dataFromBody));
  }
