...
export class TableService {
    ...
    onSort(sortMeta: SortMeta|SortMeta[]|null) : any{
        this.sortSource.next((sortMeta as any));
    }
    ...
    onTotalRecordsChange(value: number): any {
        this.totalRecordsSource.next(value);
    }

    onColumnsChange(columns: any[]): any {
...
export class SuperTable implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges {
    ...
    @Input() filterLocale: any;

    @Input() expandedRowKeys = {};

    @Input() editingRowKeys = {};
    ...
    @Input() minBufferPx: number | undefined;

    @Input() maxBufferPx: number | undefined;
    ...
    @ViewChild('container') containerViewChild: any;

    @ViewChild('resizeHelper') resizeHelperViewChild: any;

    @ViewChild('reorderIndicatorUp') reorderIndicatorUpViewChild: any;

    @ViewChild('reorderIndicatorDown') reorderIndicatorDownViewChild: any;

    @ViewChild('table') tableViewChild: any;

    @ViewChild('scrollableView') scrollableViewChild : any;

    @ViewChild('scrollableFrozenView') scrollableFrozenViewChild: any;

    @ContentChildren(PrimeTemplate) templates: any;
    ...
    filteredValue: any[] | null = null;

    headerTemplate: any;

    bodyTemplate: any;

    loadingBodyTemplate: any;

    captionTemplate: any;

    frozenRowsTemplate: any;

    footerTemplate: any;

    summaryTemplate: any;

    colGroupTemplate: any;

    expandedRowTemplate: any;

    frozenExpandedRowTemplate: any;

    frozenHeaderTemplate: any;

    frozenBodyTemplate: any;

    frozenFooterTemplate: any;

    frozenColGroupTemplate: any;

    emptyMessageTemplate: any;

    paginatorLeftTemplate: any;

    paginatorRightTemplate: any;

    paginatorDropdownItemTemplate: any;
    ...
    droppedRowIndex: number | null = null;
    ...
    editingCellRowIndex: number | null = null;

    editingCellClick= false;
    ...
    preventSelectionSetterPropagation= false;
    ...
    columnOrderStateRestored= false;

    columnWidthsState: string | null = null;

    tableWidthState: string | null = null;
    ...
    ngOnChanges(simpleChange: SimpleChanges): any {
        ...
        if (this.sortField && this.sortOrder) {
            ...
            if (this.lazy) {
                ...
                if (this.customSort) {
                    ...
                    this.value.sort((data1:any, data2:any) => {
    ...
    multisortField(data1 : any, data2: any, multiSortMeta: any, index: any) : any {
        ...
        return (multiSortMeta[index].order * (result as number));
    }

    getSortMeta(field: string):any {
                ...
                if (metaSelection) {
                    ...
                    if (selected && metaKey) {
                        if (this.isSingleSelectionMode()) {
                            ...
                            this._selection = this.selection.filter((val:any, i:any) => (i !== selectionIndex && (!!val || !val)));
                    ...
                    if (this.selectionMode === 'single') {
                        ...
                        if (selected) {
                            const selectionIndex = this.findIndexInSelection((val:any, i:any) => (i !== selectionIndex && (!!val || !val)));
                            this._selection = this.selection.filter((val:any, i:any) => (i !== selectionIndex && (!!val || !val)));
                            this.selectionChange.emit(this.selection);
                            this.onRowUnselect.emit({ originalEvent: event.originalEvent, data: rowData, type: 'row', index: event.rowIndex });
                            if (dataKeyValue) {
    ...
    handleRowTouchEnd(event: any): any {
        if (event){
            // ignore
        }
    ...
    handleRowRightClick(event: any): any {
        if (this.contextMenu) {
            const rowData = event.rowData;
    ...
    selectRange(event: MouseEvent, rowIndex: number): any {
        let rangeStart, rangeEnd;

        if ((this.anchorRowIndex as number) > rowIndex) {
            rangeStart = rowIndex;
            rangeEnd = this.anchorRowIndex;
        }
        else if ((this.anchorRowIndex as number) < rowIndex) {
        ...
        if (this.lazy && this.paginator) {
            (rangeStart as number) -= this.first;
            (rangeEnd as number) -= this.first;
        }

        const rangeRowsData = [];
        for(let i = (rangeStart as number); i <= (rangeEnd as number); i++) {
    ...
    clearSelectionRange(event: MouseEvent): any {
        let rangeStart, rangeEnd;

        if ((this.rangeRowIndex as number) > (this.anchorRowIndex as number)) {
            rangeStart = this.anchorRowIndex;
            rangeEnd = this.rangeRowIndex;
        }
        else if ((this.rangeRowIndex as number) < (this.anchorRowIndex as number)) {
        ...
        for(let i = (rangeStart as number); i <= (rangeEnd as number); i++) {
            ...
            this._selection = this.selection.filter((val:any, ix:any) => (ix !== selectionIndex && (!!val || !val)));
            const dataKeyValue: string | null = this.dataKey ? String(ObjectUtils.resolveFieldData(rangeRowData, this.dataKey)) : null;
            if (dataKeyValue) {
                delete this.selectionKeys[dataKeyValue];
            }
    ...
    findIndexInSelection(rowData: any): any {
        let index = -1;
    ...
    toggleRowWithRadio(event: any, rowData:any): any {
        this.preventSelectionSetterPropagation = true;
    ...
    toggleRowWithCheckbox(event: any, rowData: any): any {
    ...
    toggleRowsWithCheckbox(event: Event, check: boolean): any {
    ...
    filter(value: any, field: string, matchMode: string): any {
    ...
    filterGlobal(value: any, matchMode: any): any {
            ...
            if (!this.hasFilter()) {
                ...
                for (let i = 0; i < this.value.length; i++) {
                    ...
                    for (const prop in this.filters) {
                        if (Object.keys(this.filters).includes(prop) && prop !== 'global') {
                            localFiltered = true;
                    ...
                    if (this.filters['global'] && !globalMatch && globalFilterFieldsArray) {
                        for(let j = 0; j < globalFilterFieldsArray.length; j++) {
                            const globalFilterField = (globalFilterFieldsArray[j] as any).field||globalFilterFieldsArray[j];
                            globalMatch = this.filterService.filters[((this.filters['global'] as FilterMetadata) as any).matchMode](ObjectUtils.resolveFieldData(this.value[i], globalFilterField), (this.filters['global'] as FilterMetadata).value, this.filterLocale);
    ...
    createLazyLoadMetadata(): any {
        return {
            ...
            filters: this.filters,
            globalFilter: this.filters && this.filters['global'] ? (this.filters['global'] as FilterMetadata).value : null,
            multiSortMeta: this.multiSortMeta
        };
    }
    ...
    public exportCSV(options?: any):any {
        let data;
        let csv = '';
        const columns = this.frozenColumns ? [...this.frozenColumns, ...this.columns] : this.columns;
    ...
    public scrollToVirtualIndex(index: number): any {
    ...
    public scrollTo(options: any): any {
        if (this.scrollableViewChild) {
    ...
    updateEditingCell(cell: any, data: any, field: any, index: any): any {
    ...
    isEditingCellValid(): any {
        return (this.editingCell && DomHandler.find(this.editingCell, '.ng-invalid.ng-dirty').length === 0);
    }

    bindDocumentEditListener(): any {
        if (!this.documentEditListener) {
            this.documentEditListener = (event: any) => {
    ...
    unbindDocumentEditListener(): any {
        if (this.documentEditListener) {
    ...
    initRowEdit(rowData: any): any {
        const dataKeyValue = String(ObjectUtils.resolveFieldData(rowData, this.dataKey));
        this.editingRowKeys[dataKeyValue] = true;
    }

    saveRowEdit(rowData: any, rowElement: HTMLTableRowElement): any {
    ...
    cancelRowEdit(rowData: any): any {
        const dataKeyValue = String(ObjectUtils.resolveFieldData(rowData, this.dataKey));
        delete this.editingRowKeys[dataKeyValue];
    }

    toggleRow(rowData: any, event?: Event):any {
        if (!this.dataKey) {
    ...
    isSingleSelectionMode(): any {
        return this.selectionMode === 'single';
    }

    isMultipleSelectionMode(): any {
        return this.selectionMode === 'multiple';
    }

    onColumnResizeBegin(event: any): any {
    ...
    onColumnResizeEnd(event: any, column: any): any {
        if (event){
            // ignore
        }
        let delta = this.resizeHelperViewChild.nativeElement.offsetLeft - (this.lastResizerHelperX as number);
                ...
                if (nextColumn) {
                    ...
                    if (newColumnWidth > 15 && nextColumnWidth > parseInt(nextColumnMinWidth, 10)) {
                        if (this.scrollable) {
                            ...
                            const scrollableBodyTable = DomHandler.findSingle(scrollableView, '.p-datatable-scrollable-body table') || DomHandler.findSingle(scrollableView, '.p-datatable-virtual-scrollable-body table');
    ...
    setScrollableItemsWidthOnExpandResize(column: any, newColumnWidth: any, delta: any): any {
        const scrollableView = column ? this.findParentScrollableView(column) : this.containerViewChild.nativeElement;
        const scrollableBody = DomHandler.findSingle(scrollableView, '.p-datatable-scrollable-body') || DomHandler.findSingle(scrollableView, 'cdk-virtual-scroll-viewport');
        ...
        const scrollableBodyTable = DomHandler.findSingle(scrollableBody, '.p-datatable-scrollable-body table') || DomHandler.findSingle(scrollableView, 'cdk-virtual-scroll-viewport table');
        ...
        const setWidth = (container: any, table: any, width: any, isContainerInViewportx: any) => {
            if (container && table) {
                container.style.width = isContainerInViewportx ? width + DomHandler.calculateScrollbarWidth(scrollableBody) + 'px' : 'auto'
    ...
    resizeColGroup(table: any, resizeColumnIndex: any, newColumnWidth: any, nextColumnWidth: any):any {
        if (table) {
    ...
    onColumnDragStart(event: any, columnElement: any): any {
    ...
    onColumnDragEnter(event: any, dropHeader: any): any {
        if (this.reorderableColumns && this.draggedColumn && dropHeader) {
            ...
            if (this.draggedColumn !== dropHeader) {
                ...
                this.reorderIndicatorUpViewChild.nativeElement.style.top = dropHeaderOffset.top - containerOffset.top - ((this.reorderIconHeight as any) - 1) + 'px';
                this.reorderIndicatorDownViewChild.nativeElement.style.top = dropHeaderOffset.top - containerOffset.top + dropHeader.offsetHeight + 'px';

                if (event.pageX > columnCenter) {
                    this.reorderIndicatorUpViewChild.nativeElement.style.left = (targetLeft + dropHeader.offsetWidth - Math.ceil((this.reorderIconWidth as number) / 2)) + 'px';
                    this.reorderIndicatorDownViewChild.nativeElement.style.left = (targetLeft + dropHeader.offsetWidth - Math.ceil((this.reorderIconWidth as number) / 2)) + 'px';
                    this.dropPosition = 1;
                }
                else {
                    this.reorderIndicatorUpViewChild.nativeElement.style.left = (targetLeft - Math.ceil((this.reorderIconWidth as number) / 2)) + 'px';
                    this.reorderIndicatorDownViewChild.nativeElement.style.left = (targetLeft - Math.ceil((this.reorderIconWidth as number) / 2)) + 'px';
                    this.dropPosition = -1;
                }
    ...
    onColumnDragLeave(event: any): any {
    ...
    onColumnDrop(event: any, dropColumn: any): any {
        event.preventDefault();
    ...
    onRowDragStart(event: any, index: any): any {
        this.rowDragging = true;
        this.draggedRowIndex = index;
        event.dataTransfer.setData('text', 'b');    // For firefox
    }

    onRowDragOver(event: any, index: any, rowElement: any): any {
    ...
    onRowDragLeave(event: any, rowElement: any): any {
        if (event){
            // ignore
        }
        const prevRowElement = rowElement.previousElementSibling;
    ...
    onRowDrop(event: any, rowElement: any):any {
        if (this.droppedRowIndex !== null) {
            const dropIndex = ((this.draggedRowIndex as any) > this.droppedRowIndex) ? this.droppedRowIndex : (this.droppedRowIndex === 0) ? 0 : this.droppedRowIndex - 1;
            ObjectUtils.reorderArray(this.value, (this.draggedRowIndex as any), dropIndex);
    ...
    restoreState(): any {
        ...
        const reviver = function(key: any, value: any): any {
        ...
        if (stateString) {
            ...
            if (this.paginator) {
                if (this.first !== undefined) {
                    this.first = (state.first as number);
                    this.firstChange.emit(this.first);
                }

                if (this.rows !== undefined) {
                    this.rows = (state.rows as number);
                    this.rowsChange.emit(this.rows);
                }
            }
            ...
            if (state.sortField) {
                ...
                this._sortOrder = (state.sortOrder as any);
            }
            ...
            if (this.resizableColumns) {
                this.columnWidthsState = (state.columnWidths as any);
                this.tableWidthState = (state.tableWidth as any);
            }
    ...
    saveColumnWidths(state: any): any {
        const widths:any = [];
        const headers = DomHandler.find(this.containerViewChild.nativeElement, '.p-datatable-thead > tr:first-child > th');
    ...
    restoreColumnWidths(): any {
        if (this.columnWidthsState) {
            ...
            if (this.scrollable) {
                const headerCols = DomHandler.find(this.containerViewChild.nativeElement, '.p-datatable-scrollable-header-table > colgroup > col');
                const bodyCols = this.virtualScroll ? DomHandler.find(this.containerViewChild.nativeElement, 'cdk-virtual-scroll-viewport table > colgroup > col') : DomHandler.find(this.containerViewChild.nativeElement, '.p-datatable-scrollable-body table > colgroup > col');

                headerCols.map((col, index) => col.style.width = widths[index] + 'px');
                bodyCols.map((col, index) => col.style.width = widths[index] + 'px');
            }
            else {
                const headers = DomHandler.find(this.tableViewChild.nativeElement, '.p-datatable-thead > tr:first-child > th');
                headers.map((header, index) => header.style.width = widths[index] + 'px');
            }
        }
    }

    saveColumnOrder(state: any): any {
        if (this.columns) {
    ...
    restoreColumnOrder(): any {
        ...
        if (stateString) {
            ...
            if (columnOrder) {
                ...
                columnOrder.map(key =>  {
                    const col = this.findColumnByKey(key);
                    if (col) {
...
export class ScrollableView implements AfterViewInit,OnDestroy {
    ...
    @Input("pScrollableView") columns= [];

    @Input() frozen= false;

    @ViewChild('scrollHeader') scrollHeaderViewChild: any;

    @ViewChild('scrollHeaderBox') scrollHeaderBoxViewChild: any;

    @ViewChild('scrollBody') scrollBodyViewChild: any;

    @ViewChild('scrollTable') scrollTableViewChild: any;

    @ViewChild('scrollFooter') scrollFooterViewChild: any;

    @ViewChild('scrollFooterBox') scrollFooterBoxViewChild: any;

    @ViewChild('scrollableAligner') scrollableAlignerViewChild: any;

    @ViewChild(CdkVirtualScrollViewport) virtualScrollBody: CdkVirtualScrollViewport | null = null;
    ...
    frozenSiblingBody: HTMLDivElement | null = null;
    ...
    bindEvents(): any {
        this.zone.runOutsideAngular(() => {
            ...
            if (!this.frozen) {
                ...
                if (this.dt.virtualScroll)
                    (this.virtualScrollBody as any).getElementRef().nativeElement.addEventListener('scroll', this.bodyScrollListener);
                else
    ...
    onScrollIndexChange(index: number): any {
        if (this.dt.lazy) {
...
export class SortableColumn implements OnInit, OnDestroy {
    ...
    @Input("pSortableColumn") field= "";

    @Input() pSortableColumnDisabled= false;
...
export class SelectableRow implements OnInit, OnDestroy {
    ...
    @Input("pSelectableRowIndex") index= 0;
...
export class ContextMenuRow {
    ...
    @Input("pContextMenuRowIndex") index= 0;

    @Input() pContextMenuRowDisabled= false;

    selected= false;

    subscription: Subscription | undefined;

    constructor(public dt: SuperTable, public tableService: TableService, private el: ElementRef) {
        if (this.isEnabled()) {
...
export class RowToggler {
    ...
    @Input() pRowTogglerDisabled= false;
...
export class ResizableColumn implements AfterViewInit, OnDestroy {
    ...
    @Input() pResizableColumnDisabled= false;
    ...
    ngAfterViewInit() {
        if (this.isEnabled()) {
            ...
            this.zone.runOutsideAngular(() => {
                ...
                (this.resizer as any).addEventListener('mousedown', this.resizerMouseDownListener);
            });
        }
    }
    ...
    ngOnDestroy() {
        if (this.resizerMouseDownListener) {
            (this.resizer as any).removeEventListener('mousedown', this.resizerMouseDownListener);
        }
...
export class ReorderableColumn implements AfterViewInit, OnDestroy {
    ...
    @Input() pReorderableColumnDisabled= false;
...
export class EditableColumn implements AfterViewInit {
    ...
    @Input("pEditableColumnRowIndex") rowIndex= 0;

    @Input() pEditableColumnDisabled= false;

    @Input() pFocusCellSelector= "";
    ...
    closeEditingCell(completed: any, event: any) {
        if (completed)
    ...
    findPreviousEditableColumn(cell: Element) : any {
        ...
        if (!prevCell) {
            let previousRow = (cell.parentElement as any).previousElementSibling;
            if (previousRow) {
    ...
    findNextEditableColumn(cell: Element) : any {
        ...
        if (!nextCell) {
            let nextRow = (cell.parentElement as any).nextElementSibling;
            if (nextRow) {
    ...
    findPrevEditableColumnByIndex(cell: Element, index: number) {
        let prevRow = (cell.parentElement as any).previousElementSibling;
...
export class CellEditor implements AfterContentInit {
    ...
    constructor(public dt: SuperTable, @Optional() public editableColumn: EditableColumn, @Optional() public editableRow: EditableRow) { }

    ngAfterContentInit() {
        (this.templates as any).forEach((item: any) => {
            switch (item.getType()) {
                case 'input':
...
export class TableRadioButton  {
    ...
    @ViewChild('box') boxViewChild: any;
...
export class ReorderableRow implements AfterViewInit {
    ...
    @Input("pReorderableRow") index= 0;

    @Input() pReorderableRowDisabled= false;
...
export class ColumnFilterFormElement implements OnInit {
    ...
    filterCallback: Function | null = null;

    constructor(public dt: SuperTable) {}

    ngOnInit() {
        this.filterCallback = (value: any) => {
            (this.filterConstraint as any).value = value;
            this.dt._filter();
        };
    }
...
export class ColumnFilter implements AfterContentInit {
    ...
    @Input() matchModeOptions: SelectItem[] | null = null;

    @Input() maxConstraints = 2;

    @Input() minFractionDigits = 0;
    
    @Input() maxFractionDigits: any;
    ...
    generateMatchModeOptions() {
        ...
        this.config.filterMatchModeOptions[this.type]?.map((key:any) => {
        ...
        switch(event.key) {
            ...
            case 'ArrowDown':
                if (this.overlayVisible) {
                    let focusable = DomHandler.getFocusableElements(this.overlay as HTMLElement);
                    if (focusable) {
                        focusable[0].focus();
                    }
                    event.preventDefault();
    ...
    findNextItem(item: HTMLLIElement): any {
        ...
        if (nextItem)
            ...
            return (item.parentElement as any).firstElementChild;
    }
    ...
    findPrevItem(item: HTMLLIElement): any {
        ...
        return (item.parentElement as any).lastElementChild;
    }
    ...
    onOverlayAnimationStart(event: AnimationEvent) {
        switch (event.toState) {
            case 'visible':
                ...
                document.body.appendChild((this.overlay as HTMLElement));
                (this.overlay as any).style.zIndex = String(++DomHandler.zindex);
    ...
    getDefaultOperator(): string {
        return this.dt.filters ? (<FilterMetadata[]> this.dt.filters[this.field] as any)[0].operator: this.operator;
    }
    ...
    get fieldConstraints(): FilterMetadata[] | null{
    ...
    isOutsideClicked(event: any): boolean {
        return !((this.overlay as any).isSameNode(event.target) || (this.overlay as any).contains(event.target) 
...
@NgModule({
    ...
    exports: [SuperTable,SharedModule,SortableColumn,SelectableRow,RowToggler,ContextMenuRow,ResizableColumn,ReorderableColumn,EditableColumn,CellEditor,SortIcon,
    ...
    declarations: [SuperTable,SortableColumn,SelectableRow,RowToggler,ContextMenuRow,ResizableColumn,ReorderableColumn,EditableColumn,CellEditor,TableBodyComponent,ScrollableView,SortIcon,
