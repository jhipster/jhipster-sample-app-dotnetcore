...
describe("Auth JWT", () => {
  ...
  describe("Get Token", () => {
    ...
    it("should return token from session storage if local storage is empty", () => {
      sessionStorageService.retrieve = jest
        .fn()
        .mockReturnValue("sessionStorageToken");
      const result = service.getToken();
      expect(result).toEqual("sessionStorageToken");
    });

    it("should return token from localstorage storage", () => {
      localStorageService.retrieve = jest
        .fn()
        .mockReturnValue("localStorageToken");
      const result = service.getToken();
  ...
  describe("Login", () => {
    ...
    it("should clear local storage and save in session storage when rememberMe is false", () => {
      ...
      // THEN
      httpMock.verify();
      expect(sessionStorageService.store).toHaveBeenCalledWith(
        "authenticationToken",
        "1"
      );
